...
st = time.time()
func = 'powl-exp' # function for exponential decay

alpha=1 # fixed powerlaw exponent
zetalist = [0,1e-3,1e-2,0.1,1,2,3,5,10] # exp-decay exponent
...
@jit(nopython=True)
def power_law_with_exp_cutoff(dist, zeta):
    """Calculate the probability based on the exponential distribution."""
    return (dist) ** (-alpha) * math.exp(-zeta * dist)
...
def distance(i, j, zeta, L, func):
    ...
    probability = power_law_with_exp_cutoff(distance, zeta)
    return probability
...
def main(zeta, func, ini_p, end_p, step_p):
    ...
    for ti in range(runs): # take the averaged value
        ...
        # gc_size_cc.append(np.mean(temp)) # if more seeds, then averaged.
